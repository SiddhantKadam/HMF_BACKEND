...
public class BannerController {
    ...
    @GetMapping(value = "/datewiseActiveBanner")
    private ResponseEntity datewiseActiveBanner(){
        List list=bannerService.datewiseActiveBanner();
        if(!list.isEmpty()){return new ResponseEntity(list, HttpStatus.OK);}
        else{return new ResponseEntity(list,HttpStatus.INTERNAL_SERVER_ERROR);}
    }
}
